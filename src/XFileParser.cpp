...
std::vector<XToken> XFileParser::parseTokens(const uint8_t* data, size_t size, int maxTokens) {
    ...
    size_t offset = 16;  // Skip "xof 0303binn\0"
    ...
    while (offset < size && (int)tokens.size() < maxTokens) {
        ...
        switch (tokenType) {
            ...
            case 31: // Punctuation, no extra data
                break;
            ...
            case 44: case 45: offset += 1; break; // CHAR, UCHAR
            case 46: offset += 2; break; // SWORD
            case 47: offset += 4; break; // DWORD_ALT
            default: offset = size; break; // Unknown - stop
        }
