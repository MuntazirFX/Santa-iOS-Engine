...
- (void)setMeshToRender:(MeshData *)mesh {
    ...
    // Vertex layout: 3 pos + 2 uv + 3 color = 8 floats
    ...
    _vertexBuffer = [_device newBufferWithBytes:vb length:mesh.vertexCount*8*sizeof(float) options:MTLResourceStorageModeShared];
    delete[] vb;
