...
// ============ List all xof offsets (for debug) ============
+ (NSString *)findSantaModel {
    ...
    [out appendFormat:@"Total xof: %zu\n", offsets.size()];
    return out;
}

// ============ FAST: Scan all xof, find Santa, return mesh ============
+ (MeshData *)findSantaMesh {
    NSString *xpkPath = [[NSBundle mainBundle] pathForResource:@"xmas" ofType:@"xpk"];
    NSData *xpkData = [NSData dataWithContentsOfFile:xpkPath];
    ...
    // Step 1: Collect all xof offsets
    std::vector<size_t> offsets;
    for (size_t i = 0; i < totalSize - 12; i++) {
        if (bytes[i] == 'x' && bytes[i+1] == 'o' && bytes[i+2] == 'f' && bytes[i+3] == ' ') {
            offsets.push_back(i);
    ...
    NSLog(@"[Santa] Scanning %zu xof files", offsets.size());
    
    // Step 2: For each, decompress and check if it contains "weihnacht" + has a valid Mesh
    MeshData *bestMesh = nil;
    int biggestVerts = 0;
    ...
    for (size_t off : offsets) {
        @autoreleasepool {
            ...
            for (size_t k = 0; k + needleLen < decompressed.size(); k++) {
                ...
                if (match) { hasSanta = true; break; }
            }
            if (!hasSanta) continue;
            
            NSLog(@"[Santa] Found weihnacht reference at @%zu", off);
            
            // Parse tokens and extract biggest mesh
            ...
            for (size_t i = 0; i < tokens.size(); i++) {
                if (tokens[i].type == 1 && tokens[i].name == "Mesh") {
                    ...
                    biggestVerts = vc;
                    bestMesh = mesh;
                    NSLog(@"[Santa] Mesh @%zu: %d v, %d f, tex=%@", off, mesh.vertexCount, mesh.faceCount, mesh.textureName ?: @"(none)");
                }
            }
        }
    }
...
// ============ Extract mesh at a specific offset (fallback) ============
