...
// ============ NEW: Extract mesh from named asset ============
+ (MeshData *)extractMeshFromAssetNamed:(NSString *)assetName {
    NSString *xpkPath = [[NSBundle mainBundle] pathForResource:@"xmas" ofType:@"xpk"];
    if (!xpkPath) return nil;
    
    std::string xpkPathStd = [xpkPath UTF8String];
    AssetManager assetMgr;
    if (!assetMgr.loadXPK(xpkPathStd)) {
        NSLog(@"[Mesh] XPK load failed");
        return nil;
    }
    
    std::string targetName = [assetName UTF8String];
    std::vector<uint8_t> data = assetMgr.getAssetData(targetName);
    
    if (data.empty()) {
        NSLog(@"[Mesh] Asset not found: %@", assetName);
        return nil;
    }
    
    NSLog(@"[Mesh] %@: %lu bytes compressed", assetName, (unsigned long)data.size());
    
    std::vector<uint8_t> decompressed = XFileParser::decompressMSZip(data.data(), data.size());
    if (decompressed.size() < 16) {
        NSLog(@"[Mesh] Decompression failed");
        return nil;
    }
    
    NSLog(@"[Mesh] Decompressed: %lu bytes", (unsigned long)decompressed.size());
    
    std::vector<XToken> tokens = XFileParser::parseTokens(decompressed.data(), decompressed.size(), 3000);
    NSLog(@"[Mesh] %@ tokens: %lu", assetName, (unsigned long)tokens.size());
        ...
        if (tokens[i].type == 1 && tokens[i].name == "Mesh") {
            ...
            if (j < tokens.size() && tokens[j].type == 6) {
                ...
                for (size_t k = 0; k < triangles.size(); k++) {
                    if (triangles[k] < (uint32_t)mesh.vertexCount) validCount++;
                    else break;
                }
            ...
            NSLog(@"[Mesh] Extracted: %d verts, %d faces, tex=%@",
                  mesh.vertexCount, mesh.faceCount, mesh.textureName ?: @"(none)");
            return mesh;
        }
    }
    
    NSLog(@"[Mesh] No Mesh template found in %@", assetName);
    return nil;
}
