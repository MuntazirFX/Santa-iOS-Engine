...
+ (NSString *)listAssetsByKeyword:(NSString *)keyword {
    ...
    AssetManager am;
    if (!am.loadXPK([resourcePath UTF8String])) return @"fail";
    ...
    if (c == 0) [out appendFormat:@"(none)\n"];
    return out;
}

// ============ FIND ALL MESHES (no limit) ============
+ (NSString *)findAllMeshes {
    ...
    // Pehle saare X-File offsets dhoondein (no limit)
    std::vector<size_t> offsets;
    for (size_t i = 0; i < totalSize - 12; i++) {
        if (bytes[i] == 'x' && bytes[i+1] == 'o' && bytes[i+2] == 'f' && bytes[i+3] == ' ' &&
            bytes[i+4] == '0' && bytes[i+5] == '3' && bytes[i+6] == '0') {
            offsets.push_back(i);
            i += 1000; // Skip ahead to avoid duplicates
        }
    }
    
    [out appendFormat:@"Total X-Files: %zu\n", offsets.size()];
    [out appendString:@"Filtering for meshes > 100 vertices...\n\n"];
    
    // Ab har offset se mesh extract karein
    int found = 0;
    for (size_t off : offsets) {
        @autoreleasepool {
            MeshData *mesh = [self extractMeshAtOffset:off];
            if (mesh && mesh.vertexCount > 100) {
